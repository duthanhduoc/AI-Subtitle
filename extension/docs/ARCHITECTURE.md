...
Tài liệu PiP quản lý bộ điều khiển và di chuyển media từ player page. Markup và style tĩnh nằm trong `src/pip/player.html` và `src/pip/player.css`; `player.ts` gắn hành vi DOM. Player page chuyển SRT thành VTT và có thể chèn stylesheet `::cue` vào VTT theo lựa chọn của người dùng. Khi player page có VTT, video thật mang native text track cùng stylesheet sang PiP và là nguồn hiển thị phụ đề duy nhất.
