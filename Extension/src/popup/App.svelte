<script lang="ts">
	...
	async function scanHls(tabId: number, candidateId?: string) {
		...
		const attachedUrls = candidateId ? await hlsForCandidate(tabId, candidateId) : [];
	...
	async function hlsForCandidate(tabId: number, id: string) {
		...
		const marked = await ask<null>({ type: 'MARK_HLS_TARGET', id, marker }, tabId);
		if (!marked.ok) return [];
	...
	async function open() {
		...
		try {
			...
			const reply = playerTabId ? await askPlayer<null>(message, playerTabId) : await ask<null>(message);
			if (!reply.ok) throw new Error(reply.error);
