...
const setIcon = (element: Element, icon: IconNode) =>
...
export async function openPlayer(
  ...
  if (!api)
    throw new Error(
      'Document Picture-in-Picture is not supported by this browser.'
    )
  const parent = video.parentNode
  if (!parent)
    throw new Error('The selected video is no longer attached to this page.')
  ...
  const restore = () => {
    ...
    if (original.parent.isConnected)
      original.parent.insertBefore(
        video,
        original.next?.isConnected ? original.next : null
      )
  ...
  const fontSize = (
    { small: '18px', medium: '24px', large: '30px', xlarge: '36px' } as const
  )[settings.fontSize ?? 'medium']
  const alpha = ({ off: 0, low: 0.25, medium: 0.5, high: 0.75 } as const)[
  ...
  const $ = <T extends Element>(selector: string) =>
    body.querySelector(selector) as T
  ...
  for (const chevron of menu.querySelectorAll('.menu-chevron'))
    setIcon(chevron, ChevronRight)
  for (const back of menu.querySelectorAll('.back-icon'))
    setIcon(back, ArrowLeft)
  ...
  const refreshTracks = () => {
    trackValue.textContent =
      tracks[subtitleState.activeTrackIndex ?? -1]?.name ?? 'Off'
  ...
  const addTracks = (incoming: StoredSubtitle[]) => {
    ...
    for (const track of incoming)
      if (
        !tracks.some(
          (saved) => saved.name === track.name && saved.text === track.text
        )
      )
        tracks.push(track)
    selectTrack(
      tracks.findIndex(
        (track) =>
          track.name === incoming[0]?.name && track.text === incoming[0]?.text
      )
    )
  }
  const refreshSpeeds = () => {
  ...
  const render = () => {
    ...
    subtitle.textContent = activeTrack
      ? (findCue(activeTrack.cues, video.currentTime + offset)?.text ?? '')
      : ''
  ...
  menu
    ...
    .forEach((button) =>
      button.addEventListener('click', () =>
        choosePreset(button.dataset.preset as SubtitlePreset)
      )
    )
  ...
  menu
    .querySelectorAll<HTMLButtonElement>('.choice-back')
    .forEach((button) =>
  ...
  srtFile.addEventListener('change', () => {
    ...
    void Promise.all(
      [...(srtFile.files ?? [])]
        .filter((file) => file.name.toLowerCase().endsWith('.srt'))
