...
let sequence = 0;
const idFor = (video: HTMLVideoElement) =>
  ids.get(video) ?? (ids.set(video, `video-${++sequence}`), ids.get(video)!);
...
function videos(root: ParentNode = document): HTMLVideoElement[] {
  ...
  for (const el of root.querySelectorAll("*"))
    if (el.shadowRoot) result.push(...videos(el.shadowRoot));
  for (const frame of root.querySelectorAll("iframe"))
    if (frame.contentDocument) result.push(...videos(frame.contentDocument));
  return result;
}
